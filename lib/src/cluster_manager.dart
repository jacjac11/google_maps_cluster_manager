...
class CClusterManager<T extends CClusterItem> {
  CClusterManager(this._items, this.updateMarkers,
      {Future<Marker> Function(CCluster<T>)? markerBuilder,
  ...
  Future<List<CCluster<T>>> getMarkers() async {
    ...
    if (stopClusteringZoom != null && _zoom >= stopClusteringZoom!)
      return visibleItems.map((i) => CCluster<T>.fromItems([i])).toList();
  ...
  List<CCluster<T>> _computeClustersWithMaxDist(
  ...
  List<CCluster<T>> _computeClusters(
      List<T> inputItems, List<CCluster<T>> markerItems,
      {int level = 5}) {
  ...
  static Future<Marker> Function(CCluster) get _basicMarkerBuilder =>
      (cluster) async {
        return Marker(
